...
export function saveWithTTL(key, data, ttlMinutes = 360) {
  try {
    const item = {
      data,
      timestamp: Date.now(),
      ttl: ttlMinutes * 60 * 1000, // переводим в миллисекунды
...
// Кеширование астрологических событий (12 часов)
export function saveAstroEvents(data) {
  const key = 'astro_events';
  saveWithTTL(key, data, 720); // 12 часов
}

export function loadAstroEvents() {
  return loadWithTTL('astro_events');
}
...
export function clearAllCache() {
  try {
    ...
    console.log(`🗑️ Очищен весь кеш: ${keys.length} элементов`);
    return keys.length;
  } catch (error) {
...
// Запуск автоматической очистки при загрузке
const initCacheCleanup = () => {
  // Очищаем просроченные элементы при загрузке
  cleanExpiredCache();
...
if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
  window.cacheDebug = {
    ...
    saveHoro: saveHoroscope,
    loadHoro: loadHoroscope
  };
